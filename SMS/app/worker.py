...
def send_sms2(phone):  # thông báo khóa 2c
    ...
    Trân trọng!”"""
    print(f"[SMS] Gửi đến {phone}: {message}")
#http://10.159.26.88:9000/itel/sps-can-subscriber
...
def check_transaction_status(sdt: str):
    session = SessionLocal()
    job = session.query(Transaction.status, Transaction.transaction_date).filter(PhoneCheckInfo.sdt == sdt)
    jobs = session.query(PhoneCheckInfo).filter(PhoneCheckInfo.sdt == sdt)
    if Transaction.status == 1 and  jobs.run_date <=Transaction.transaction_date <= jobs.run_date + timedelta(days=34):
        job.is_update = 1
        session.commit()
    return job.is_update
...
def process_jobs1():
    ...
    for job2 in jobs2:
        if(check_transaction_status(job2.sdt) == 1):
        ...
        try:
            if(job.status == PhoneCheckStatus.UPDATED and job.run_date <= now <= job.run_date + timedelta(days=29)):
                ...
                if(job.run_date <= now <= job.run_date + timedelta(days=4) or job.run_date + timedelta(days=12) <= now <= job.run_date + timedelta(days=14)):
                    ...
                    threading.Timer(delay, send_sms1, args=[job.sdt]).start()
                ...
                if(job.run_date + timedelta(days=27) <= now <= job.run_date + timedelta(days=29)):
                    ...
                    delay = (rand_time - now).total_seconds()
                    threading.Timer(delay, send_sms2, args=[job.sdt]).start()
                    print(f"Đã gửi SMS tới {job.sdt} lúc:, {rand_time}")
